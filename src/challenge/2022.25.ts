...
export class Solution extends BaseSolution {
    ...
    public solvePart1(lines: Input): string {
        return decimalToSnafu(sum(lines.map(snafuToDecimal)) ?? 0).toString();
    }
...
const snafuToDecimal = (s: string) =>
    s ? snafuToDecimal(s.slice(0, -1)) * 5 + valueOf(s[s.length - 1]) : 0;

const decimalToSnafu = (d: number): string =>
    d ? decimalToSnafu(Math.floor((d + 2) / 5)).concat('=-012'.charAt((d + 2) % 5)) : '';

const valueOf = (char: string): number => (char ? '=-012'.indexOf(char) - 2 : NaN);
