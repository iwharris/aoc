...
function popFirst(input: NumberArray): number {
...
function parseHeader(input: NumberArray): Header {
...
function parseMetadata(input: NumberArray, count: number): Metadata {
...
function parseInputToTree(lines: string[]): NavigationSystemTree {
    const input: NumberArray = lines[0].split(' ').map((n) => parseInt(n));
