...
export class Solution extends BaseSolution {
    ...
    public solvePart1(lines: Input): string {
        ...
        let result = 0;
        let m = 0;
        const o = new Array(TIME_LIMIT + 1)
            .fill(0)
            .map((_, t) => Math.abs(Math.floor((t - 1) * t) / 2));
        ...
        for (const [
            ...
            const mj = obsidianRobotClayCost;
            const mk = geodeRobotObsidianCost;

            const dfs = (
                timeRemaining: number,
                goalRobotType: number, // t:time remaining, g:goal robot type
                numOreRobots: number,
                numClayRobots: number,
                numObsidianRobots: number,
                numGeodeRobots: number, // i:ore, j:clay, k:obsidian, l:geode robots
                availableOreRobots: number,
                availableClayRobots: number,
                availableObsidianRobots: number,
                availableGeodeRobots: number
            ) => {
                if (
                    (goalRobotType == BOT_ORE && numOreRobots >= mi) ||
                    (goalRobotType == BOT_CLAY && numClayRobots >= mj) ||
                    (goalRobotType == BOT_OBSIDIAN &&
                        (numObsidianRobots >= mk || numClayRobots == 0)) ||
                    (goalRobotType == BOT_GEODE && numObsidianRobots == 0) ||
                    availableGeodeRobots + numGeodeRobots * timeRemaining + o[timeRemaining] <= m
                ) {
                    return;
                }
                while (timeRemaining) {
                    if (goalRobotType == 0 && availableOreRobots >= oreRobotOreCost) {
                        for (let robotType: Robot = 0; robotType < 4; robotType++) {
                            dfs(
                                timeRemaining - 1,
                                robotType,
                                numOreRobots + 1,
                                ...
                                numObsidianRobots,
                                numGeodeRobots,
                                availableOreRobots - oreRobotOreCost + numOreRobots,
                                availableClayRobots + numClayRobots,
                                availableObsidianRobots + numObsidianRobots,
                                availableGeodeRobots + numGeodeRobots
                            );
                        }
                        return;
                    } else if (goalRobotType == 1 && availableOreRobots >= clayRobotOreCost) {
                        for (let robotType = 0; robotType < 4; robotType++) {
                            dfs(
                                ...
                                numObsidianRobots,
                                numGeodeRobots,
                                availableOreRobots - clayRobotOreCost + numOreRobots,
                                availableClayRobots + numClayRobots,
                                availableObsidianRobots + numObsidianRobots,
                                availableGeodeRobots + numGeodeRobots
                            );
                        ...
                        goalRobotType == 2 &&
                        availableOreRobots >= obsidianRobotOreCost &&
                        availableClayRobots >= obsidianRobotClayCost
                    ) {
                        for (let robotType = 0; robotType < 4; robotType++) {
                            dfs(
                                timeRemaining - 1,
                                ...
                                numObsidianRobots + 1,
                                numGeodeRobots,
                                availableOreRobots - obsidianRobotOreCost + numOreRobots,
                                availableClayRobots - obsidianRobotClayCost + numClayRobots,
                                availableObsidianRobots + numObsidianRobots,
                                availableGeodeRobots + numGeodeRobots
                            );
                        }
                        ...
                        goalRobotType == 3 &&
                        availableOreRobots >= geodeRobotOreCost &&
                        availableObsidianRobots >= geodeRobotObsidianCost
                    ) {
                        for (let robotType = 0; robotType < 4; robotType++) {
                            dfs(
                                timeRemaining - 1,
                                ...
                                numObsidianRobots,
                                numGeodeRobots + 1,
                                availableOreRobots - geodeRobotOreCost + numOreRobots,
                                availableClayRobots + numClayRobots,
                                availableObsidianRobots -
                                    geodeRobotObsidianCost +
                                    numObsidianRobots,
                                availableGeodeRobots + numGeodeRobots
                            );
                    ...
                    timeRemaining -= 1;
                    availableOreRobots += numOreRobots;
                    availableClayRobots += numClayRobots;
                    availableObsidianRobots += numObsidianRobots;
                    availableGeodeRobots += numGeodeRobots;
                }
                m = Math.max(m, availableGeodeRobots);
            };

            for (let robotType = 0; robotType < 4; robotType += 1) {
                dfs(TIME_LIMIT, robotType, 1, 0, 0, 0, 0, 0, 0, 0);
            }
            result += m * blueprintId;
        }
...
// const evaluateBlueprint = (blueprint: Blueprint): number => {};

type Blueprint = [
    number, // id
    number, // ore_bot cost (ore)
    number, // clay_bot cost (ore)
    number, // obsidian_bot cost (ore)
    number, // obsidian_bot cost (clay)
    number, // geode_bot cost (ore)
    number // geode_bot cost (obsidian)
];

const parseBlueprints = (lines: Input): Blueprint[] => {
    return lines.map((line) => {
        const m = [...line.matchAll(/(\d+)/g)].map((matches) => matches[0]).map((v) => parseInt(v));
        return m as Blueprint;
    });
};

const BOT_ORE = 0;
const BOT_CLAY = 1;
const BOT_OBSIDIAN = 2;
const BOT_GEODE = 3;

type Robot = typeof BOT_ORE | typeof BOT_CLAY | typeof BOT_OBSIDIAN | typeof BOT_GEODE;
