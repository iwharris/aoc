...
type Passport = Record<PassportField | string, string>;
...
const validators: Record<PassportField | string, Validator> = {
    ...
    hgt: (val) => {
        ...
        const rawHeight = matches[1];
        const unit = matches[2];

        // console.log(rawHeight, unit)
...
const passportContainsRequiredFields = (passport: Passport): boolean => {
    const required: PassportField[] = ['byr', 'ecl', 'eyr', 'hcl', 'hgt', 'iyr', 'pid'];

    return required.every((field) => !!passport[field]);
};

const passportIsValid = (passport: Passport, idx: number): boolean => {
    // console.log(`passport ${idx}: ${JSON.stringify(passport)}`);
    const isRequiredFieldsPresent = passportContainsRequiredFields(passport);
    const allValidatorsPass = Object.keys(passport).every((key) => {
        const validator = validators[key];
        const result = validator(passport[key]);
        // console.log(`validator ${key} returned ${result}`);
        return result;
    });

    // console.log(`passport ${idx} isRequiredFields=${isRequiredFieldsPresent} allValidators=${allValidatorsPass}`);

    return isRequiredFieldsPresent && allValidatorsPass;
};
