...
export class Solution extends BaseSolution {
    ...
    solvePart1(lines: Input): string {
        const emu = new BitmaskEmulator();

        lines.forEach((l) => emu.parseLine(l));

        // console.log(emu.memory)

        return sum(Array.from(emu.memory.values())).toString();
    }

    solvePart2(lines: Input): string {
        const emu = new BitmaskEmulator2();
...
class BitmaskEmulator {
    ...
    parseLine(line: string): void {
        if (line.startsWith('mask')) {
            // load new mask
            const matches = line.match(/^mask = ([X01]{36})$/);
            if (!matches) throw new Error();
            this.mask = matches[1];
        } else if (line.startsWith('mem')) {
            const matches = line.match(/^mem\[(\d+)\] = (\d+)$/);
            if (!matches) throw new Error();
            const address = parseInt(matches[1]);
            const value = parseInt(matches[2]);
            // console.log(`value is ${value}`)
            const maskedValue = this.applyMask(value);
            // console.log(`maskedValue is ${maskedValue}`)
            this.memory.set(address, maskedValue);
        }
    }

    applyMask(val: number): number {
        const valMask = val.toString(2).padStart(36, '0');

        const newValMask = Array.from(valMask)
        ...
        // console.log(`after mask is ${newValMask}`)

        return parseInt(newValMask, 2);
    }
}
