...
export class Solution extends BaseSolution {
    ...
    public solvePart2(lines: Input): string {
        return sum(
            parseTrios(lines).map((trio) => {
                const itemArrays = trio.map(parseBagToItemArray);

                return sum(
                    itemArrays[0].map((_, i) => {
                        return itemArrays.every((arr) => arr[i] > 0) ? priorityOfIndex(i) : 0;
                    })
                );
            })
        ).toString();
...
type RucksackTrio = [string, string, string];
const parseTrios = (lines: Input): Array<RucksackTrio> => {
    const trios: Array<RucksackTrio> = [];
    for (let i = 0; i < lines.length; i += 3) {
        trios.push(lines.slice(i, i + 3) as RucksackTrio);
    }
    return trios;
};
